...
async function getDb() {
  if (!db) {
    ...
    // ─── Seed Demo Data ─────────────────────────────────────────────────────
    const seedCheck = await db.get('SELECT COUNT(*) as c FROM users');
    if (seedCheck.c === 0) {
      const hash = bcrypt.hashSync('password123', 10);

      const r1 = await db.run(
        'INSERT INTO users (name, email, password_hash, phone, role) VALUES (?, ?, ?, ?, ?)',
        ['Rajesh Kumar', 'rajesh@example.com', hash, '9876543210', 'user']
      );
      const r2 = await db.run(
        'INSERT INTO users (name, email, password_hash, phone, role) VALUES (?, ?, ?, ?, ?)',
        ['Priya Sharma', 'priya@example.com', hash, '9123456789', 'user']
      );
      await db.run(
        'INSERT INTO users (name, email, password_hash, phone, role) VALUES (?, ?, ?, ?, ?)',
        ['Arjun Patel', 'arjun@example.com', hash, '9988776655', 'user']
      );

      const owner1 = r1.lastID;
      const owner2 = r2.lastID;
